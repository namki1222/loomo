...
### Added
- **Hub chat — talk to your hub from inside the dashboard.** A new **Hub** tab shows the hub's conversation and sends what you type straight to it, so you can delegate work without switching to its terminal. It is the same hub session, not a second agent: the history is the one the hub already has, and it still routes work to your project panes. Replies appear as the hub writes them (per message, not per token). Starts the hub session automatically if it isn't running.
