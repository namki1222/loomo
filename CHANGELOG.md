...
### Added
- **Hub tab — your hub as a real pane in the dashboard.** The dashboard now runs as a pane of loomo's own tmux server, which lets the **Hub** tab borrow the hub's actual pane into the window: you type into Claude's own prompt, natively, with its live output and formatting. Leaving the tab (or closing the dashboard) hands the pane back. Where that isn't possible — an older launch, or tmux unavailable — the tab mirrors the pane instead and forwards your keystrokes to it.
- **Hub tab — reach your hub from the dashboard.** A new **Hub** tab shows the hub's recent turns and, with **[대화 열기]**, opens the hub's own Claude screen right there — full streaming and Claude's own formatting, because it is that session, not a copy. Detach (`Ctrl-b d`) returns to the dashboard. For a one-liner you can also type into the dashboard's input line without opening it. The hub session starts automatically if it isn't running.
- **Pick the model every pane runs.** **Settings → AI models** now shows a `Model` toggle under Claude and Codex — click to cycle it (Claude: `auto → opus → sonnet → haiku`, Codex: `auto → gpt-5-codex → gpt-5`), and every pane loomo opens launches with that model. `auto` leaves the choice to the CLI, as before. Override per machine with `LOOMO_CLAUDE_MODEL` / `LOOMO_CODEX_MODEL` (any model name or alias the CLI accepts).
