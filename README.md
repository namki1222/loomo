...
- **tmux 세션 = 프로젝트 팀.** 세션 하나가 프로젝트 하나다 (`proj-a`, `blog`, …).
- **패널(pane) = 그 팀에 상주하는 AI 동료.** 패널 제목이 곧 역할이자 주소다 (`server`, `web`, `infra`). 각 패널에서 Claude Code가 **장수명으로** 돌아간다 — 스폰됐다 사라지는 서브에이전트가 아니라, 그 코드베이스의 이력과 맥락을 계속 들고 있는 담당자다.
...
"연결" 버튼 같은 건 없다. **tmux에 떠 있다는 것 자체가 연결이다.**
...
- **유리箱 오케스트레이션** — 모든 대화가 tmux 화면에 그대로 보인다. 블랙박스 없음. 언제든 사람이 끼어들 수 있다.
- **인프라 제로** — `tmux send-keys`로 상대 입력창에 타이핑하는 게 전부다. 데몬·큐·MCP 서버·훅 설정이 없다. 전체 코드가 bash 한 파일이라 5분이면 다 읽힌다.
...
- 허브(비서) 세션 이름/역할/디렉터리를 묻는다 → 엔터만 치면 `hub`/`hub`/`~/tell-hub`
- 이어서 프로젝트 세션을 추가한다. 예: 세션 `demo` → 역할 `api` → 디렉터리 `~/work/demo/api` → (엔터로 종료)
- 이때 **각 디렉터리의 CLAUDE.md에 협업 규약이 자동 삽입**된다 — 받는 쪽 Claude가 "어떻게 응답해야 하는지"를 아는 근거다.

**2. 세션 띄우기**

```bash
tell ws hub        # 허브 세션 (Claude 자동 실행)
tell ws demo       # 프로젝트 세션 — 등록된 역할 수만큼 패널 분할 + 각각 Claude 실행
```

**3. 첫 핑퐁** — 허브 패널의 Claude에게 입력:

```
tell demo api "핑퐁 테스트. 받은 키로 tell -r 로 '퐁 정상' 한 줄만 응답해줘"
```

몇 초 뒤 허브 입력창에 이렇게 돌아오면 성공이다:

```
[세션 응답 - 3f9a1c from demo/api] 퐁 정상
```

이 순간부터 두 세션은 서로에게 일을 시킬 수 있다.

## 빠른 시작 B — 이미 쓰던 세션 편입 (`tell adopt`)

**이미 tmux 안에서 Claude Code를 쓰고 있다면** — 재시작 없이, 대화 컨텍스트를 유지한 채 그대로 편입된다.
...
1. 떠 있는 tmux 패널을 전부 스캔해서 하나씩 보여준다
...
> tmux 밖(일반 터미널 탭)에서 쓰던 세션은? tmux 패널을 하나 만들고 그 안에서 `claude --resume` 으로 기존 대화를 이어받으면 된다. 그 다음 `adopt`.
...
P2P 대화가 코어지만, 프로젝트가 여럿이면 **허브 세션 하나**를 두는 걸 권장한다:
...
| `tell init` | 신규 셋업 마법사 — 허브 + 프로젝트 등록 + CLAUDE.md 규약 삽입 |
| `tell adopt` | 떠 있는 tmux 패널 스캔 → 역할 지정 → 규약 삽입 → (선택) 로드 메시지 전송 |
