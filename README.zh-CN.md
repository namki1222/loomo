...
- 查看 Hub 最近的对话，并用 **[대화 열기]** 直接打开 Hub 自己的 Claude 界面 —— 因为那就是同一个会话而非副本，流式输出与 Claude 原生排版都完整保留。按 `Ctrl-b d` 回到仪表盘。
- 只想让它做一件小事时，直接在仪表盘输入行里输入即可，会原样转给 Hub。
- 如果 Hub 会话没有运行，会自动启动。
...
这同时会把协作约定插入每个目录的约定文件（`CLAUDE.md` 或 `AGENTS.md`）——这正是接收方 AI 通过桥回复的依据。窗格也可以直接向其他项目的角色发消息；跨越多个项目的工作交给中枢即可。
